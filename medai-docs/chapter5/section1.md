...
初次使用 MedAI 时，你可能对于应用创建比较陌生。为了帮助新手用户快速了解在 MedAI 上能够构建哪些类型的应用，MedAI 团队内的提示词工程师已经创建好了多场景、高质量的应用模板。
...
如果你需要在 MedAI 上创建一个空白应用，你可以从导航选择 「工作室」 ，在应用列表内选择 「从空白创建 」。

![img](../images/chapter5-003.jpg)

初次创建应用时，你可能需要先理解 MedAI 上 4 种不同应用类型的[基本概念](https://docs.dify.ai/v/zh-hans/guides/application-orchestrate#application_type)，分别是聊天助手、文本生成应用、Agent 和工作流。
...
MedAI DSL 是由 MedAI.AI 所定义的 AI 应用工程文件标准，文件格式为 YML。该标准涵盖应用在 MedAI 内的基本描述、模型参数、编排配置等信息。
